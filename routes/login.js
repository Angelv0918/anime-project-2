...
router.get('/new-acc', (req, res) => {
  res.render('new-acc.ejs', {title:'New Account', loggedIn:false, error:null});
});
...
router.post('/new', async(req, res) => {
  ...
  if (email != '' && pass != '') {
      ...
      if (!data) {
        ...
        let passSalt = await bcrypt.genSalt(saltRounds, async(err, salt) => {
          let passHash = await bcrypt.hash(pass, salt, async(err, hash) => {
            let acc = {email:email, pwd:hash, level:'admin'};
            let newUser = new schemas.users(acc);
    ...
    res.render('new-acc.ejs', {title:'New Account', loggedIn:false, error:'All fields are required. Please check and try again.'});
  }
});
